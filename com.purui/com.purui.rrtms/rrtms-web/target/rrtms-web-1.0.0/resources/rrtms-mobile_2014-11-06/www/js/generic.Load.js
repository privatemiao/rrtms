...
generic.Load = function(obj) {
	$.extend(this, obj);

	this.$filterContext = $('#page-load-filter');
	this.$startDate = this.$filterContext.find('#load-start-date');
	this.$endDate = this.$filterContext.find('#load-end-date');
	this.$btnQuery = this.$filterContext.find('.btn-query');
	this.$btnCancelQuery = this.$filterContext.find('.btn-cancel');
	this.init();
};

generic.Load.prototype = {
	destroy : function() {
	...
	eventComponent : function() {
		...
		(function(reference) {
			reference.$filterContext.find('[name=type-load]').unbind('click').click(function() {
		...
		(function(reference) {
			reference.$btnQuery.unbind('click').click(function() {
				...
				data.type = reference.$filterContext.find('[name=type-load]:checked').val();
					...
					response.forEach(function(e) {
						reference.chart.addSeries({
							...
							data : (function() {
								var d = [];
								var count = 0;
								e.totalLoads.forEach(function(load) {
	...
	resizeChart : function() {
		if (this.chart) {
			...
			this.chart.setSize(this.$context.find('#chart-load').parent().width(), height, doAnimation = true);
		}
	}
