var draw2d = {
	...
	ui : {},
	isTouchDevice : ((navigator.platform.indexOf("iPhone") != -1) || (navigator.platform.indexOf("iPod") != -1) || (navigator.platform.indexOf("iPad") != -1))
};
...
draw2d.layout.connection.JunctionRouter = draw2d.layout.connection.ConnectionRouter.extend({
	...
	route : function(connection, oldJunctionPoints) {
		...
		var end = connection.getEndPoint();
		connection.addPoint(start);
		connection.addPoint(oldJunctionPoints.get(1));
		connection.addPoint(end);
			...
			intersectionForCalc.each($.proxy(function(ii, interP) {
				if (draw2d.shape.basic.Line.hit(1, oldP.x, oldP.y, p.x, p.y, interP.x, interP.y) === true) {
					if (conn.sharingPorts(interP.other)) {
						...
						if (connZ < otherZ) {
							var junctionNode = conn.canvas.paper.ellipse(interP.x, interP.y, this.junctionRadius, this.junctionRadius).attr({
								fill : conn.lineColor.hash()
							});
...
draw2d.layout.connection.MazeConnectionRouter = draw2d.layout.connection.ConnectionRouter
		.extend({
			...
			_route : function(conn, fromPt, fromDir, toPt, toDir) {
				...
				if (this.useDebug) {
					...
					ROUTER_RECTS.push(conn.canvas.paper.path(svgPathBefore).attr({
						"stroke" : "#0000ff"
					}));
				}
			...
			simplifyRadialDistance : function(points, sqTolerance) {
				var i, len = points.length, point, prevPoint = points[0], newPoints = [ prevPoint ];
				for (i = 1; i < len; i++) {
					point = points[i];
...
draw2d.policy.canvas.SnapToGeometryEditPolicy = draw2d.policy.canvas.SnapToEditPolicy.extend({
	...
	populateRowsAndCols : function() {
		...
		for ( var i = 0; i < figures.getSize(); i++) {
			...
			if (!selection.contains(figure)) {
				...
				this.cols[index * 3] = {
					type : -1,
					location : bounds.getX()
				};
				this.rows[index * 3] = {
					type : -1,
					location : bounds.getY()
				};
				this.cols[index * 3 + 1] = {
					type : 0,
					location : bounds.x + (bounds.getWidth() - 1) / 2
				};
				this.rows[index * 3 + 1] = {
					type : 0,
					location : bounds.y + (bounds.getHeight() - 1) / 2
				};
				this.cols[index * 3 + 2] = {
					type : 1,
					location : bounds.getRight() - 1
				};
				this.rows[index * 3 + 2] = {
					type : 1,
					location : bounds.getBottom() - 1
				};
				index++;
			}
		}
	...
	showVerticalLine : function(x) {
		...
		this.vline = this.canvas.paper.path("M " + x + " 0 l 0 " + this.canvas.getHeight()).attr({
	...
	showHorizontalLine : function(y) {
		...
		this.hline = this.canvas.paper.path("M 0 " + y + " l " + this.canvas.getWidth() + " 0").attr({
			"stroke" : this.LINE_COLOR,
			"stroke-width" : 1
		});
	},
...
draw2d.policy.line.JunctionSelectionFeedbackPolicy = draw2d.policy.line.LineSelectionFeedbackPolicy.extend({
	...
	onSelect : function(canvas, connection, isPrimarySelection) {
		...
		for ( var i = 1; i < (points.getSize() - 1); i++) {
	...
	moved : function(canvas, figure) {
		...
		for ( var i = 2; i < figure.selectionHandles.getSize(); i++) {
			console.log("MOVE JUNCTION CONN");
			var handle = figure.selectionHandles.get(i);
			var point = points.get(i - 1);
			var resizeWidthHalf = handle.getWidth() / 2;
			var resizeHeightHalf = handle.getHeight() / 2;
			handle.setPosition(point.x - resizeWidthHalf, point.y - resizeHeightHalf);
		}
	}
});
...
draw2d.SetFigure = draw2d.shape.basic.Rectangle.extend({
	...
	applyTransformation : function() {
		var s = "S" + this.scaleX + "," + this.scaleY + ",0,0 " + "R" + this.rotationAngle + "," + ((this.getWidth() / 2) | 0) + "," + ((this.getHeight() / 2) | 0) + "T" + this.getAbsoluteX() + ","
				+ this.getAbsoluteY() + "";
...
draw2d.Connection = draw2d.shape.basic.PolyLine.extend({
	...
	repaint : function(attributes) {
		...
		if (this.sourceDecoratorNode !== null) {
			...
			this.sourceDecoratorNode.node.setAttribute("class", this.cssClass !== null ? this.cssClass : "");
...
draw2d.shape.basic.JunctionResizeHandle = draw2d.shape.basic.LineResizeHandle.extend({
	...
	onDrag : function(dx, dy) {
		...
		var diffX = oldX - this.getX();
		var diffY = oldY - this.getY();
		var objPos = this.owner.getPoints().get(this.index);
		objPos.setPosition(objPos.x - diffX, objPos.y - diffY);
		this.owner.isMoving = true;
		this.detachMoveListener(this.owner);
		this.owner.routingRequired = true;
		this.owner.repaint();
		return true;
	},
	onDrop : function(dropTarget) {
...
draw2d.shape.arrow.CalligrapherArrowLeft = draw2d.SVGFigure
		.extend({
...
draw2d.shape.arrow.CalligrapherArrowDownLeft = draw2d.SVGFigure
		.extend({
...
draw2d.shape.analog.OpAmp = draw2d.SVGFigure
		.extend({
			...
			getSVG : function() {
				return '<svg xmlns="http://www.w3.org/2000/svg" version="1.1">'
						...
						+ '<line x1="0.53516"  y1="8"  x2="8.21191"  y2="8"  stroke="#010101"/>' + '<line x1="39.14941" y1="18" x2="45.81055" y2="18" stroke="#010101" />'
						+ '<line x1="0.53516"  y1="27" x2="8.21191"  y2="27" stroke="#010101" />' + "</svg>";
			},
			repaint : function(attributes) {
				if (this.repaintBlocked === true || this.shape === null) {
					return;
				...
				if (this.bgColor != null) {
					this.svgNodes[0].attr({
						fill : this.bgColor.hash()
					});
				}
				this._super(attributes);
			}
		});
draw2d.shape.analog.ResistorBridge = draw2d.SVGFigure
		.extend({
...
draw2d.shape.analog.VoltageSupplyHorizontal = draw2d.SVGFigure
		.extend({
...
draw2d.shape.analog.VoltageSupplyVertical = draw2d.SVGFigure
		.extend({
...
draw2d.shape.icon.Icon = draw2d.SetFigure.extend({
	...
	applyTransformation : function() {
		if (this.isResizeable() === true) {
			this.svgNodes.transform("S" + this.scaleX + "," + this.scaleY + "," + this.getAbsoluteX() + "," + this.getAbsoluteY() + "t" + (this.getAbsoluteX() - this.offsetX) + ","
					+ (this.getAbsoluteY() - this.offsetY));
		} else {
...
draw2d.shape.icon.Thunder = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Snow = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Hail = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Rain = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Cloudy = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Sun = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Undo = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Detour = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Merge = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Split = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Fork = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.ForkAlt = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Shuffle = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Refresh = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Ccw = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Acw = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Contract = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Expand = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Smile2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Smile = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Alarm = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Clock = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.StopWatch = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.History = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Future = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.GlobeAlt2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.GlobeAlt = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Globe = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Warning = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Pensil = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Pen = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Sticker = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Page2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Page = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Landscape1 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Landscape2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Plugin = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Bookmark = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Hammer = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Users = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.User = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Customer = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Employee = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Anonymous = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Skull = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Picture = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Bubble = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.CodeTalk = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Talkq = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Talke = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Lock = draw2d.shape.icon.Icon
		.extend({
			...
			createSet : function() {
				return this.canvas.paper
						.path("M22.335,12.833V9.999h-0.001C22.333,6.501,19.498,3.666,16,3.666S9.666,6.502,9.666,10h0v2.833H7.375V25h17.25V12.833H22.335zM11.667,10C11.667,10,11.667,10,11.667,10c0-2.39,1.944-4.334,4.333-4.334c2.391,0,4.335,1.944,4.335,4.333c0,0,0,0,0,0v2.834h-8.668V10z");
			}
		});
draw2d.shape.icon.Clip = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.StarOff = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Star2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Star2Off = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Star3 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Star3Off = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Chat = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Quote = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Gear2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Wrench = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Wrench2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Wrench3 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.ScrewDriver = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.HammerAndScrewDriver = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Magic = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Download = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.View = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Noview = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Cloud = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Cloud2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.CloudDown = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.CloudUp = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Location = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Volume1 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Volume2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Volume3 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Key = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Ruler = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Power = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Unlock = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Flag = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Search = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.ZoomOut = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.ZoomIn = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Settings = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.SettingsAlt = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Feed = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Bug = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Link = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Calendar = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.No = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Photo = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Printer = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Export = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Import = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Run = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Magnet = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.NoMagnet = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.ReflectH = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.ReflectV = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Resize2 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Rotate = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Connect = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Disconnect = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Man = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Woman = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.People = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Parent = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Notebook = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Diagram = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.PieChart = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.LineChart = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Apps = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Locked = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Ppt = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Lab = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Umbrella = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Dry = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Ipad = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Iphone = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Jigsaw = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Lamp = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Lamp_alt = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Video = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Palm = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Fave = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Help = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Crop = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.BioHazard = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.WheelChair = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Mic = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.MicMute = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.IMac = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Pc = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Cube = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.FullCube = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Font = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Trash = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Pallete = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Cart = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Glasses = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Package = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Book = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Books = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Db = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Paper = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.TakeOff = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Landing = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Plane = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Phone = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.HangUp = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.SlideShare = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Twitter = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.TwitterBird = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Skype = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Windows = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Apple = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Linux = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.NodeJs = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.JQuery = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Sencha = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Vim = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.InkScape = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Aumade = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Firefox = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Ie = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Ie9 = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Opera = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Chrome = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Safari = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.LinkedIn = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Flickr = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.GitHub = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.GitHubAlt = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Raphael = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.GRaphael = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Svg = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Usb = draw2d.shape.icon.Icon
		.extend({
...
draw2d.shape.icon.Ethernet = draw2d.shape.icon.Icon
		.extend({
...
draw2d.decoration.connection.DiamondDecorator = draw2d.decoration.connection.Decorator.extend({
	...
	paint : function(paper) {
		var st = paper.set();
		st.push(paper.path([ "M", this.width / 2, " ", -this.height / 2, "L", this.width, " ", 0, "L", this.width / 2, " ", this.height / 2, "L", 0, " ", 0, "L", this.width / 2, " ",
				-this.height / 2, "Z" ].join("")));
		st.attr({
			fill : this.backgroundColor.hash(),
			stroke : this.color.hash()
		});
...
(function() {
	Raphael.fn.group = function(f, g) {
		...
		this.group.rotate = function(c, a, e) {
			this.setAttribute("transform", "translate(" + this.getAttr("translate").x + "," + this.getAttr("translate").y + ") scale(" + this.getAttr("scale").x + "," + this.getAttr("scale").y
					+ ") rotate(" + c + "," + a + "," + e + ")");
		};
