$(function() {
	...
	com.rrtms.system.analysis.EnergyCompare = function() {
		this.COMPARE_BY = {
			AREA : 'area',
			INDUSTRY : 'industry'
		};
	...
	com.rrtms.system.analysis.EnergyCompare.prototype = {
		destroy : function() {
			console.log('destroy EnergyCompare...');
		...
		initTree : function(kind) {
			...
			this.tree = new dhtmlXTreeObject('compare-tree', '100%', '100%', '-1');
		...
		eventComponent : function() {
			...
			$('#type').change(function(e) {
				...
				switch (val) {
					case reference.TYPE.DAY:
						$('#startDate').attr('title', hide ? '日期' : '开始日期');
						$('#startDate').attr('data-original-title', hide ? '日期' : '开始日期');

						$('#endDate').attr('title', '结束日期');
						$('#endDate').attr('data-original-title', '结束日期');

						console.log('create date picker', val);
						reference.startDate = $('#startDate').kendoDatePicker({
							format : "yyyy-MM-dd"
						}).data('kendoDatePicker');
						if (!$('#endDate').is(':hidden')) {
							reference.endDate = $('#endDate').kendoDatePicker({
								format : "yyyy-MM-dd"
							}).data('kendoDatePicker');
						}
						break;
					case reference.TYPE.MONTH:
						$('#startDate').attr('title', hide ? '月份' : '开始月份');
						$('#startDate').attr('data-original-title', hide ? '月份' : '开始月份');

						$('#endDate').attr('title', '结束月份');
						$('#endDate').attr('data-original-title', '结束月份');
					...
					case reference.TYPE.YEAR:
						$('#startDate').attr('title', hide ? '年份' : '开始年份');
						$('#startDate').attr('data-original-title', hide ? '年份' : '开始年份');

						$('#endDate').attr('title', '结束年份');
						$('#endDate').attr('data-original-title', '结束年份');
			...
			$('.btn-okay').click(function() {
				...
				$.ajax({
					url : '../../system/analysis/energy/compare',
					data : data,
					error : handleError,
					traditional : true,
					success : function(response) {
							...
							reference.chart.addSeries({
								name : (function() {
									...
									if (data.compareBy == 'industry') {
										var _name = reference.tree.getItemText(name);
										name = _name == '0' ? name : _name;
									}

									if (!data.compareByValue || data.compareByValue.length == 1) {
										switch (data.type) {
											case "day":
