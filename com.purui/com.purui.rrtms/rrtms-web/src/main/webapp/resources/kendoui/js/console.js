(function($) {
	var count = 0, oldMessage;

	window.kendoConsole = {
		log : function(message, isError) {
			var oldContainer = $(".console div:first"), counter = oldContainer.find(".count");
			...
			if (!oldContainer.length || message != oldMessage) {
				...
				}).html(message).prependTo(".console").animate({
					marginTop : 0
				}, 300).animate({
				...
				if (counter.length) {
					...
					oldContainer.html(oldMessage).append("<span class='count'>" + count + "</span>");
				}
			}

			var total = $(".console>div");
			if (total.length > $.CONSOLE_MAX_COUNT) {
		...
		jQuery.fx.step[attr] = function(fx) {
			...
			fx.elem.style[attr] = [
					...
					[ Math.max(Math.min(parseInt((fx.pos * (fx.end[0] - fx.start[0])) + fx.start[0], 10), 255), 0),
							...
							Math.max(Math.min(parseInt((fx.pos * (fx.end[2] - fx.start[2])) + fx.start[2], 10), 255), 0) ].join(","), ")" ].join("");
		};
	});
