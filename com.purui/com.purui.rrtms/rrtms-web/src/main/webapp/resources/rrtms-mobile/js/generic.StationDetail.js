...
generic.StationDetail.prototype = {
			...
			generic.ajax({
				...
				final : function(response) {
					...
					response.data.forEach(function(warn) {
						...
						warn.date = dateTimeFormat5(new Date(warn.atTime));
					});
	...
	generateChart : function(response) {
		...
		this.$chart.style.width = plus.display.resolutionWidth + 'px';
		this.$chart.style.height = plus.display.resolutionWidth + 'px';

		var g = new Dygraph(document.getElementById("chart"), function() {
			var count = 1, data = [];
			response[0].data.forEach(function(e) {
				data.push([ count, e.totalValue, (response[1].data[count] ? response[1].data[count].totalValue : null) ]);
				console.log([ count, e.totalValue, (response[1].data[count] ? response[1].data[count].totalValue : null) ]);
				count++;
			});
			return data;
		}, {
