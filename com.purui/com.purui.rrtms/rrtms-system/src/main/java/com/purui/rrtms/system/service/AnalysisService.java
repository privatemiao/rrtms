...
public class AnalysisService {
	...
	protected Logger logger = LoggerFactory.getLogger(AnalysisService.class);

	// 负荷对比
	public List<AnalysisLoadWrapper> compareLoad(AnalysisActionParam param) {

		List<AnalysisLoadWrapper> wrappers = new ArrayList<>();
		...
		case COMPARE_DATE_TYPE_DAY:
			logger.debug("COMPARE_DATE_TYPE_DAY");
		...
		case COMPARE_DATE_TYPE_MONTH:
			logger.debug("COMPARE_DATE_TYPE_MONTH");
			if (param.getCompareByValue() == null || param.getCompareByValue().length == 0) {
				// 比对所有区域的不同两个日期的值
		...
		case COMPARE_DATE_TYPE_YEAR:
			logger.debug("COMPARE_DATE_TYPE_YEAR");
			if (param.getCompareByValue() == null || param.getCompareByValue().length == 0) {
				// 比对所有区域的不同两个日期的值
	...
	private TotalLoad groupTotalLoad(MongoTemplate mongoTemplate, Criteria criteria, String groupBy, String groupByValue) {
//		@formatter:off
...
//		@formatter:on
		...
		GroupByResults<TotalLoad> results = mongoTemplate.group(criteria, "TotalFuHe", group, TotalLoad.class);
		Iterator<TotalLoad> iterator = results.iterator();

		TotalLoad load = null;
		if (StringUtils.isBlank(groupByValue)) {
			load = new TotalLoad();
			while (iterator.hasNext()) {
				TotalLoad next = iterator.next();
				load.setCount(load.getCount() + next.getCount());
				load.setTotalValue(load.getTotalValue() + next.getTotalValue());
			}
		} else {
			if (iterator.hasNext()) {
				load = iterator.next();
	...
	private AnalysisLoadWrapper loadOneYear(String groupBy, String groupByValue, Date date) {
		...
		AnalysisLoadWrapper wrapper = new AnalysisLoadWrapper();
		List<TotalLoad> loads = new ArrayList<>();

		for (int i = 1; i <= 12; i++) {
			Criteria criteria = Criteria.where("mMonth").is(i);
			if (!StringUtils.isBlank(groupByValue)) {
...
//			MongoTemplate mongoTemplate = mongoDBUtil.getMongoTemplate("PUHUA_" + c.get(Calendar.YEAR));
			...
			loads.add(groupTotalLoad(mongoTemplate, criteria, groupBy, groupByValue));
		}

		wrapper.setName(groupByValue == null ? "总平台" : groupByValue);
		wrapper.setTotalLoads(loads);
		wrapper.setDate(date);
		return wrapper;
	}

	private AnalysisLoadWrapper loadOneMonth(String groupBy, String groupByValue, Date date) {
		Calendar c = Calendar.getInstance();
		c.setTime(date);
		c.set(Calendar.HOUR_OF_DAY, 0);
		c.set(Calendar.MINUTE, 0);
		...
		for (int i = 1; i <= c.getActualMaximum(Calendar.DAY_OF_MONTH); i++) {
			Criteria criteria = Criteria.where("mMonth").is(c.get(Calendar.MONTH) + 1).and("mDay").is(i);
			if (!StringUtils.isBlank(groupByValue)) {
				criteria.and(groupBy).is(groupByValue);
			}
//			MongoTemplate mongoTemplate = mongoDBUtil.getMongoTemplate("PUHUA_" + c.get(Calendar.YEAR));
	 ...
	 */
	private AnalysisLoadWrapper loadOneDay(String groupBy, String groupByValue, Date date) {
		Calendar c = Calendar.getInstance();
		c.setTime(date);
		c.set(Calendar.HOUR_OF_DAY, 0);
		c.set(Calendar.MINUTE, 0);
		...
		for (int i = 0; i < 24; i++) {
			...
			Criteria criteria = Criteria.where("Atime").gte(_startDate).andOperator(Criteria.where("Atime").lt(_endDate));
			if (!StringUtils.isBlank(groupByValue)) {
				criteria.and(groupBy).is(groupByValue);
			}
...
//			MongoTemplate mongoTemplate = mongoDBUtil.getMongoTemplate("PUHUA_" + c.get(Calendar.YEAR));
	...
	public Point powerIndicator(AnalysisPowerIndicatorParam param) {
		Point point = baseDao.load(Point.class, param.getPointId());
		List<String> codeFilter = prepareCodeFilter(param);
		Calendar c = Calendar.getInstance();
		...
		for (int i = point.getDataPoints().size() - 1; i >= 0; i--) {
			...
			Query query = new Query(Criteria.where("TagGuid").is(dp.getGuid()).and("mDay").is(day)).with(new Sort(Sort.Direction.ASC, "Atime"));
	...
	private List<String> prepareCodeFilter(AnalysisPowerIndicatorParam param) {
		List<String> codeFilter = new ArrayList<>();

		switch (param.getCurve()) {
		case "1":
			for (String k : param.getKinds()) {
				switch (k) {
				case "A":
					codeFilter.add("4");
					break;
